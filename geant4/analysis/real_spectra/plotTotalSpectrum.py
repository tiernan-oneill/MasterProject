...
'''
This script is used to make one plot: the total real energy spectrum of all of the isotopes. 
...
for Isotope in IsotopeDict:
    ...
    exec(f"root_file_{Isotope} = ROOT.TFile.Open(f'/data/xenon/toneill/rdecay01_edit_2_output/output_{Isotope}.root')")
    ...
    exec(f"hist_{Isotope} = ROOT.TH1F('hist_{Isotope}', 'Isotope Energy Spectrum', n_bins, x_min, x_max)")
    exec(f"total_tree_{Isotope}.Draw('totalEkin>>hist_{Isotope}')")

print("Finalising plot...")

canvas = ROOT.TCanvas("canvas", "Isotope Energy Spectrum", 1000, 600)
...
output_file_name = "total_real_energy.png"
folder_path = "figures"
...
output_root_file = ROOT.TFile("total_real_energy_hist.root", "RECREATE")
hist_combined.Write()
