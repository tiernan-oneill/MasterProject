...
'''
This script is used to make one plot: the total visible energy spectrum of all of the isotopes. 
...
for Isotope in IsotopeDict:
    ...
    exec(f"hist_{Isotope} = ROOT.TH1F('hist_{Isotope}', 'Isotope Visible Energy Spectrum', n_bins, x_min, x_max)")
    exec(f"total_tree_{Isotope}.Draw('smeared_calibrated_energy>>hist_{Isotope}')")

print("Finalising plot...")

canvas = ROOT.TCanvas("canvas", "Isotope Visible Energy Spectrum", 1000, 600)
...
output_file_name = "total_visible_spectrum.png"
folder_path = "figures"
...
output_root_file = ROOT.TFile("total_visible_energy_hist.root", "RECREATE")
hist_combined.Write()
